...
__global__ void kernComputeIndices(int N, int gridResolution,
	...
	int3 cell;
	cell.x = floor((boid_pos.x - gridMin.x) * inverseCellWidth);
	cell.y = floor((boid_pos.y - gridMin.y) * inverseCellWidth);
	cell.z = floor((boid_pos.z - gridMin.z) * inverseCellWidth);

	cell.x = imax(0, imin(gridResolution - 1, cell.x));
	cell.y = imax(0, imin(gridResolution - 1, cell.y));
	cell.z = imax(0, imin(gridResolution - 1, cell.z));
...
__global__ void kernIdentifyCellStartEnd(int N, int* particleGridIndices,
	...
	int current_grid_index = particleGridIndices[index];

	if (index == 0 || particleGridIndices[index - 1] != current_grid_index) {
		gridCellStartIndices[current_grid_index] = index;
	}

	if (index == N - 1 || particleGridIndices[index + 1] != current_grid_index) {
		gridCellEndIndices[current_grid_index] = index + 1;
	}
}
...
__global__ void kernUpdateVelNeighborSearchScattered(
	...
	int self_orig_index = particleArrayIndices[index];
	glm::vec3 my_pos = pos[self_orig_index];
	glm::vec3 my_vel = vel1[self_orig_index];
	...
	int3 self_cell = make_int3(
		(my_pos.x - gridMin.x) * inverseCellWidth,
		(my_pos.y - gridMin.y) * inverseCellWidth,
		(my_pos.z - gridMin.z) * inverseCellWidth
	);
			...
			for (int x = -1; x <= 1; ++x) {
				...
				if (start != -1) {
					for (int i = start; i < end; ++i) {
						int neighbor_orig_index = particleArrayIndices[i];
						if (self_orig_index == neighbor_orig_index) continue;

						glm::vec3 neighbor_pos = pos[neighbor_orig_index];
						glm::vec3 neighbor_vel = vel1[neighbor_orig_index];
...
__global__ void kernReshuffleParticleData(int N, int* particleArrayIndices,
	...
	pos_out[index] = pos_in[originalIndex];
	vel_out[index] = vel_in[originalIndex];
}
...
void Boids::stepSimulationCoherentGrid(float dt) {
	...
	kernIdentifyCellStartEnd <<<fullBlocksPerGrid, blockSize >>> (numObjects, dev_particleGridIndices, dev_gridCellStartIndices, dev_gridCellEndIndices);
	kernReshuffleParticleData <<<fullBlocksPerGrid, blockSize >>> (numObjects, dev_particleArrayIndices, dev_pos, dev_vel1, dev_pos_coherent, dev_vel1_coherent);
	...
	cudaMemcpy(dev_vel1, dev_vel2_coherent, numObjects * sizeof(glm::vec3), cudaMemcpyDeviceToDevice);

	glm::vec3* temp_vel = dev_vel1_coherent;
	dev_vel1_coherent = dev_vel2_coherent;
	dev_vel2_coherent = temp_vel;
}
