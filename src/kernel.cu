...
#define GridLoopingOptimization 0
...
__device__ glm::vec3 computeVelocityChange(int N, int iSelf, const glm::vec3 *pos, const glm::vec3 *vel) {
  ...
  if (neighbor_count > 0) {
      perceived_center /= (float)neighbor_count;
  }
  ...
  if (neighbor_count > 0) {
      perceived_velocity /= (float)neighbor_count;
  }

  return rule1Scale * (perceived_center - pos[iSelf]) +
         rule2Scale * c +
...
__global__ void kernComputeIndices(int N, int gridResolution,
  ...
  glm::vec3 *pos, int *indices, int *gridIndices) {
    ...
    int cx = static_cast<int>(floorf((p.x - gridMin.x) * inverseCellWidth));
    int cy = static_cast<int>(floorf((p.y - gridMin.y) * inverseCellWidth));
    int cz = static_cast<int>(floorf((p.z - gridMin.z) * inverseCellWidth));

    cx = imax(0, imin(gridResolution - 1, cx));
    cy = imax(0, imin(gridResolution - 1, cy));
    cz = imax(0, imin(gridResolution - 1, cz));

    int cellId = gridIndex3Dto1D(cx, cy, cz, gridResolution);

    indices[index] = index;
	gridIndices[index] = cellId;
}
...
__global__ void kernUpdateVelNeighborSearchScattered(
  ...
  // - Clamp the speed change before putting the new speed in vel2
    ...
    float cx_f = (p.x - gridMin.x) * inverseCellWidth;
    float cy_f = (p.y - gridMin.y) * inverseCellWidth;
    float cz_f = (p.z - gridMin.z) * inverseCellWidth;

    int cx = static_cast<int>(floorf(cx_f));
    int cy = static_cast<int>(floorf(cy_f));
    int cz = static_cast<int>(floorf(cz_f));
    ...
    if (GridLoopingOptimization) {
        
        const float r1 = rule1Distance * rule1Distance;
        const float r2 = rule2Distance * rule2Distance;
        const float r3 = rule3Distance * rule3Distance;

        int minX = (int)floorf(((p.x - R) - gridMin.x) * inverseCellWidth);
        int maxX = (int)floorf(((p.x + R) - gridMin.x) * inverseCellWidth);
        int minY = (int)floorf(((p.y - R) - gridMin.y) * inverseCellWidth);
        int maxY = (int)floorf(((p.y + R) - gridMin.y) * inverseCellWidth);
        int minZ = (int)floorf(((p.z - R) - gridMin.z) * inverseCellWidth);
        int maxZ = (int)floorf(((p.z + R) - gridMin.z) * inverseCellWidth);

        minX = imax(0, imin(gridResolution - 1, minX));
        maxX = imax(0, imin(gridResolution - 1, maxX));
        minY = imax(0, imin(gridResolution - 1, minY));
        maxY = imax(0, imin(gridResolution - 1, maxY));
        minZ = imax(0, imin(gridResolution - 1, minZ));
        maxZ = imax(0, imin(gridResolution - 1, maxZ));
            ...
            for (int ny = minY; ny <= maxY; ++ny) {
                for (int nx = minX; nx <= maxX; ++nx) {
                    ...
                    for (int idx = startIdx; idx <= endIdx; ++idx) {  
                        ...
                        glm::vec3 d = pos[boidIndex] - p;
                        float d2 = dot(d, d);
                        if (d2 < r1) { perceived_center += pos[boidIndex]; ++n1; }
                        if (d2 < r2) { c -= d; }
                        if (d2 < r3) { perceived_velocity += vel1[boidIndex]; ++n3; }
                    }
                }
            }
        }
    ...
    if (n3 > 0) {
        perceived_velocity /= (float)n3;
    }
    glm::vec3 thisVel = vel1[index];
    glm::vec3 newVel = thisVel +
        rule1Scale * (perceived_center - p) +
        rule2Scale * c +
        rule3Scale * perceived_velocity;
    float speed = glm::length(newVel);
    if (speed > maxSpeed) {
...
__global__ void kernReshuffleCoherent(
    int N,
    const int* __restrict__ particleArrayIndices,
    const glm::vec3* __restrict__ pos,
    const glm::vec3* __restrict__ vel,
    glm::vec3* __restrict__ posCoherent,
    glm::vec3* __restrict__ velCoherent)
{
    int k = blockIdx.x * blockDim.x + threadIdx.x;
    if (k >= N) return;
    int orig = particleArrayIndices[k];
    posCoherent[k] = pos[orig];
    velCoherent[k] = vel[orig];
}
...
void Boids::stepSimulationScatteredGrid(float dt) {
  ...
  // - Ping-pong buffers as needed
	dim3 fullBlocksPerGrid((numObjects + blockSize - 1) / blockSize);
	dim3 fullBlocksCellGrid((gridCellCount + blockSize - 1) / blockSize);
    ...
    kernResetIntBuffer <<<fullBlocksCellGrid, blockSize >>> 
...
void Boids::stepSimulationCoherentGrid(float dt) {
  ...
  // - Ping-pong buffers as needed. THIS MAY BE DIFFERENT FROM BEFORE.
    ...
    kernReshuffleCoherent <<<fullBlocksPerGrid, blockSize >>>
        (numObjects,
        dev_particleArrayIndices,
        dev_pos, dev_vel1,
        dev_pos_coherent, dev_vel1_coherent);
    checkCUDAErrorWithLine("kernReshuffleCoherent failed!");
    ...
    kernUpdatePos <<<fullBlocksPerGrid, blockSize >>>
        (numObjects, dt, dev_pos_coherent, dev_vel2_coherent);
